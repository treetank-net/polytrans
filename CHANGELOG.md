...
### Changed
- Hardened the shell-runner CI workspace: Docker checks now work outside the GitLab checkout, and a scoped recovery step cleans stale root-owned dependency artifacts from earlier runs.

### Added
- Added an architecture test that prevents future Docker jobs from reintroducing checkout permission failures.

### Note
- No PolyTrans runtime behavior changed in this maintenance release.
...
### Fixed
- Disabled the one-off `MetaCleanup` migration on `admin_init`, so it no longer runs on every admin or AJAX request.
- Fixed the assistant table schema format passed to `dbDelta()`, preventing invalid `DESCRIBE IF` and empty-index `ALTER TABLE` queries.
...
- **Relayed translations are now recorded as such.** A `pl → en → de` path costs one call per hop, and the row for each hop now carries every fact about it: the language it read, the language it produced, the language the whole request was for, the full path (`pl>en>de`) and the hop's position in it. Nothing about the relay has to be inferred from timing or guessed at afterwards.
  ...
  - **By target language** now groups by the language the request was for, so the `pl→en` hop of a `pl→en→de` request counts towards German. Previously it counted towards English, crediting a market nobody ordered while understating the one that did. The per-hop truth stays in the table row, so any other view can still be built from it.
  - Post meta on the original buckets per market for the same reason. The reports accept `language` and `relay_only` filters, and grouping is available on `source_language`, `final_language`, `translation_path`, `path_step` and the derived `language_pair`.
  - The table migrates itself on the first write after an update (no reactivation needed) and backfills `final_language` on existing rows, so the whole history stays visible in a per-market view rather than dropping out of it.

### Fixed
- **Translations recorded no model, so none of them could be priced.** The model was read from the assistant's configuration, but an assistant usually has none of its own and inherits the provider default — leaving an empty string, which no price list matches, and which the dashboard could only report as *unpriced*. It is now read from the provider's response, which also names the concrete dated build rather than the alias that was requested (`gpt-4o-mini-2024-07-18`, not `gpt-4o-mini`), falling back to the configured model and then the provider default. Workflow steps were unaffected, which is why only translations showed up unpriced.
- The dashboard explains *unpriced* rather than just flagging it: the note says the total is short by whatever those calls cost, and names both causes (model absent from the price list, or no model reported at all). A group with no value reads *not reported* instead of `(none)`.
...
- One-off cleanup of the meta earlier versions left behind (`Core\MetaCleanup`, runs once per site in batches). A copy is identified by checking the pointer, per language: `_polytrans_translation_post_id_de` on a post either names a translation that post actually commissioned — in which case it is a legitimate relay hub (`pl → en → de`) and stays — or it names someone else's translation, which means the post inherited it. Keys for one language are treated as an indivisible group, since deleting a status while leaving its pointer would make the post look like it commissioned a translation it did not. Where no pointer exists the group is compared against the original and removed only when identical; anything else is reported and left alone.
  - Verified against a real database of 72 translations first, which is how the criterion got rewritten before release: the obvious rule ("remove keys naming the post's own language") turned out to remove nothing at all, because the copy is taken before the original records the status for the language being produced, so a copy never carries its own language. The real orphans are always keys for *other* languages — the ones that rule deliberately kept.
  - Cost meta is rebuilt from the `polytrans_usage` table rather than deleted, since a translation can have costs of its own from workflow steps.
...
### Fixed
- Workflow execution (automatic, manual, and background) now detects and passes `source_language` from the original post via Polylang, fixing empty `source_language` in prompts and incorrect article language context.
...
### Changed
- Default prompt refinement evaluator and adjuster prompts are now much shorter while preserving scoring, criteria, history/regression handling, Twig safety, and prompt-pack output blocks.

### Fixed
- Workflow list priority column now has an explicit width so it remains visible in the fixed-width admin table.

## [1.14.4] - 2026-05-07

### Added
- Workflow completion logs now include compact execution diagnostics with per-step timings, output variables, processed output actions, change counts, context keys, trigger, post IDs, and target language.
- Background manual workflow execution logs now include total execution time and per-step summaries.
...
### Added
- Workflows now have a numeric priority field. Lower values run earlier when multiple workflows match a translation.

### Changed
- Workflow lists and automatic translation-completed execution now sort by priority first, then workflow name for stable ordering.

## [1.14.1] - 2026-05-07

### Fixed
- Post-processing workflow chains now invalidate translated post data caches before and after each workflow execution, so following workflows see changes persisted by earlier workflow output actions.
...
### Fixed
- Prompt refinement score comparison tables now include the final verification score for the last adjusted prompt version.
...
### Fixed
- Prompt refinement score extraction now handles Markdown score labels, `/100` fractions, JSON score fields, nested evaluation scores, and Polish labels.
- Score extraction no longer treats numbered-list markers like `1)` as evaluator scores, preventing distorted average scores.
